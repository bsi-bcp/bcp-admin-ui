...
- [ ] **F08** 任务列表：名称前增加行号列（不可编辑，自动递增）
- [ ] **F09** 任务列表：表头上方增加任务统计（总数 / 启用数 / 禁用数）
- [ ] **F10** 任务列表：禁用任务整行增加淡灰色背景
- [ ] **F11** 编辑对话框：顶部表单每行放两个字段（集成名称+客户、模板选择+集成节点）
- [ ] **F12** 输入节点对话框：定时设置+数据源合并为一行，脚本区域放大
- [ ] **F13** 输出节点对话框：同 F12，数据源+访问路径合并为一行
- [ ] **F14** Cron 表达式：替换为友好的可视化 Cron 控件
- [ ] **F15** Monaco 编辑器：增加"格式化"按钮
- [ ] **F16** Monaco 编辑器：增加"复制代码"按钮（与格式化放一起）
- [ ] **F17** 参数表格：参数名称旁增加"获取参数"按钮，点击复制 `context.getParams().get("参数名")` 到剪贴板
- [ ] **F18** 输入/输出节点对话框：数据源下拉框旁增加"管理数据源"链接，点击跳转到数据源配置编辑页面
- [ ] **F19** 参数表格：删除参数时增加确认提示（"是否删除该参数？"）
- [ ] **F20** 参数表格：表头"参数数据"改为"参数值"

### Bug

- [ ] **B01** 数据源下拉框显示 ID 而非名称（admin 登录时）
